...
/**
 * The new UAE Dirham symbol (introduced 2025): a "D" with two horizontal bars.
 * Sized in em units so it scales with surrounding text; uses currentColor.
 */
export function DirhamGlyph({ className }: { className?: string }) {
  return (
    <svg
...
/**
 * Render a monetary value. For AED it shows the Dirham symbol before the
 * number; for other currencies it falls back to the ISO code (e.g. "USD 1,200.00").
 */
export function Amount({
  ...
  return <span className="whitespace-nowrap">{formatMoney(value, currency)}</span>;
}
