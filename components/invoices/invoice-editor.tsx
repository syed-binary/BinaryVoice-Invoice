...
export type EditorClient = Pick<
  Client,
  "id" | "displayName" | "currency" | "exportClient"
...
export function InvoiceEditor({
  ...
  const initialClient = clients.find(
    (c) => c.id === (invoice?.clientId ?? preselectClientId),
  );

  const [clientId, setClientId] = useState(invoice?.clientId ?? preselectClientId ?? "");
  const [issueDate, setIssueDate] = useState(invoice?.issueDate ?? today);
  const [dueDate, setDueDate] = useState(
    invoice?.dueDate ??
      (() => {
        const d = new Date();
        d.setDate(d.getDate() + company.defaultDueDays);
  ...
  const [vatRate, setVatRate] = useState(
    invoice?.vatRate ??
      (initialClient?.exportClient ? 0 : toNumber(company.defaultVatRate)),
  );
  const [withholdingEnabled, setWithholdingEnabled] = useState(
    invoice?.withholdingEnabled ?? initialClient?.withholdingRate != null,
  );
  const [withholdingRate, setWithholdingRate] = useState(
    invoice?.withholdingRate ??
      initialClient?.withholdingRate ??
      toNumber(company.defaultWithholdingRate),
  );
  const [discountType, setDiscountType] = useState<"PERCENT" | "FIXED">(invoice?.discountType ?? "FIXED");
  const [discountValue, setDiscountValue] = useState(invoice?.discountValue ?? 0);
  const [notes, setNotes] = useState(invoice?.notes ?? company.defaultNotes ?? "");
  const [terms, setTerms] = useState(invoice?.terms ?? company.defaultTerms ?? "");
  const [templateId, setTemplateId] = useState(invoice?.templateId ?? company.defaultTemplate);
  const [accentColor, setAccentColor] = useState(invoice?.accentColor ?? company.accentColor);
  const [customFields, setCustomFields] = useState<Record<string, string>>(() => {
    const cf = invoice?.customFields ?? {};
    const out: Record<string, string> = {};
    for (const d of fieldDefs) out[d.key] = cf[d.key] != null ? String(cf[d.key]) : "";
    return out;
  });
  const [lines, setLines] = useState<Line[]>(
    invoice?.lineItems.map((l) => ({ ...l, key: uid() })) ?? [blankLine()],
  );
  ...
  function submit(markSent: boolean) {
    ...
    startTransition(async () => {
      ...
      toast.success(editing ? "Invoice saved" : "Invoice created");
